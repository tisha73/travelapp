...
Flight_RoundTrip = pd.read_excel('notebooks\\HetviFlight.xlsx', engine='openpyxl')  # Replace with your flight dataset file path
...
# flight_oneway['Departure_date'] = pd.to_datetime(flight_oneway['Departure_date'], format='%d/%m/%Y').dt.strftime('%Y-%m-%d')
...
# Flight_RoundTrip['EstimatedDuration_HHMM'] = Flight_RoundTrip['EstimatedDuration_HHMM'].str.split(':').apply(lambda x: int(x[0]) * 60 + int(x[1]))
# Iterate through the rows of the DataFrame and insert data into MySQL
for i, row in Flight_RoundTrip.iterrows():
    cursor.execute("""
        INSERT INTO Test_flight ( Departure, Arrival, Departure_date, flight_number,
                    flightType, Flight_agency)
        VALUES ( %s, %s, %s, %s, %s, %s)
    """, ( row['Departure'], row['Arrival'], row['Departure_date'], row['flight_number'],
           row['flightType'], row['agency']))
